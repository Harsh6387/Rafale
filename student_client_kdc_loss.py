...
# Load Teacher mode similar to teacher_client model
...
class KD_Loss_Cosine(nn.Module):
    def __init__(self, temperature=2.0, alpha=0.5, gamma=0.2):
        super(KD_Loss_Cosine, self).__init__()
    ...
    def forward(self, student_logits, teacher_logits, labels):
        ...
        # KL Divergence Loss with softened probabilities
        ...
        cosine_sim = self.cosine_loss(student_logits, teacher_logits)
        loss_cos = 1 - cosine_sim.mean()

        # Combined Loss
        loss = self.alpha * loss_ce + (1. - self.alpha) * loss_kl + self.gamma * loss_cos
        return loss


def train_student(teacher_model, student_model, train_loader, epochs=10, temperature=2.0, alpha=0.5, gamma=1):
    teacher_model.to(device)
    teacher_model.eval()
   # Teacher model is in eval mode
    student_model.to(device)
    student_model.train()
    criterion = KD_Loss_Cosine(temperature=temperature, alpha=alpha, gamma=gamma)
...
class CifarClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        train_student(teacher, student_model , train_loader, epochs=15,temperature=2.0, alpha=0.5, gamma=1)
